...
class ImageAugmentation():
    def __init__(self, train_dir = opt.dest_train, val_dir = opt.dest_valid, gt_dir = opt.gt_dir, images_dir = opt.images_dir, log_dir = opt.log_dir):
        ...
        train_gt_dir = self.train_dir + self.gt_dir
        train_images_dir = self.train_dir + self.images_dir
        train_tensorboard_dir = self.train_dir + log_dir
        val_gt_dir = self.val_dir + self.gt_dir
        val_images_dir = self.val_dir + self.images_dir
        val_tensorboard_dir = self.val_dir + self.log_dir
        ...
        for files in (gt_files, img_files):
            ...
            for filename in files:
                ...
                if (pattern.search(filename)):
                    ...
                    dest_dir = self.train_dir + dir_name[idx]
                    ...
                    dest_dir = self.val_dir + dir_name[idx]
                    src_path = os.getcwd() + opt.source_path + '/' + dir_name[idx] + '/' + filename
                    self.crop_im(dest_dir, src_path, filename, opt.crop_size, k)
            idx += 1   
